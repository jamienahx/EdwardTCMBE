...
return await userSchema.create(user); //create the user in daos
};
const loginUser=async(user)=> {
    ...
    const userFound = await userSchema.findOne({ email: user.email, password: user.password });
  ...
  //update the user record with this new jwt for session tracking
    await userSchema.updateOne({ email: user.email }, { jwt: token });
...
async function logoutUser(email) {
    //clears out the stored jwt token and invalidates their session.
        await userSchema.updateOne({email}, {jwt: ""});
    }
